...
def test_some_alarms_linked_to_lots():
    """lot 처리 구간과 겹치는 알람이 존재하는 이상, lot_id가 전부 None이면 안 된다."""
...
def test_linked_alarm_ts_within_lot_window():
    """연결된 알람의 시각은 그 lot이 해당 장비를 지나던 구간 안이어야 한다."""
...
def test_unlinked_alarm_has_no_containing_window():
    """lot_id 없는 알람은 실제로 어느 lot 처리 구간에도 안 들어야 한다 (유휴 중 알람)."""
...
def test_scenario_cause_alarms_visible_in_lot_timeline():
    """시나리오 원인 알람('out of range')이 최소 1건은 어떤 lot 타임라인에 잡혀야 한다."""
